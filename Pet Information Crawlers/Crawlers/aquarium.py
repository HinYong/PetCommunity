...
def task(success, fail):
    ...
    def parserDetail(item, name, image, item_url):
        ...
        # 没有英文名的情况
            ...
            English_name_nospace = "aquarium" + str(count)
        # 服务器上的存储路径
        dirpath = "../../../后端/PetCommunity_Backend/src/main/resources/static/crawler_images/aquariums/" + English_name_nospace + "/"
        # 因为路径有中文，所以需要解码
        ...
        with open(main_image, 'wb') as f:
            f.write(r.content)
        main_image = "http://localhost:8081/crawler_images/aquariums/" + English_name_nospace + "/" + English_name_nospace + ".png"
        print main_image
        time.sleep(5)
        ...
        for i in imageList:
            ...
            swiper_images = swiper_images + "http://localhost:8081/crawler_images/aquariums/" + English_name_nospace + "/" \
        ...
        # 插入数据库
        util.insert(pet_type_id=4, name=name, English_name=English_name, character=character,
    ...
    def aquariums_crawler():
        isLoop = True
        old_list = None
        # 查询列表页
        pageNo = 1  # 页码
        while isLoop:
            pageUrl = "http://www.boqii.com/pet-all/aquarium/"
            if pageNo > 1:
                pageUrl = "http://www.boqii.com/pet-all/aquarium/?p=%d" % pageNo
